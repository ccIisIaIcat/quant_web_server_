package spot_goods

import (
	"context"
	"encoding/json"
	"fmt"
	"log"
	"os"
	"sync"
	"time"

	"github.com/gorilla/websocket"
)

type Orderbook_info struct {
	Instrument_id         string                      //币对
	Infogather_processed  chan map[string]string      //用于存放处理完的orderbook信息
	infogather_original   chan []byte                 //用于存放数据的channal
	websocket_address     string                      //websocket地址
	connect               *websocket.Conn             //websocket连接
	ctx                   context.Context             //更为优雅的停止向channel中输入数据
	cancel                context.CancelFunc          //更为优雅的停止向channel中输入数据
	wg                    sync.WaitGroup              //用于优雅的结束进程
	logger                *log.Logger                 //用于输出日志文件
	orderbook_struct_temp orderbook_infogather_struct //用于存放当前orderbook结构体
}

//初始化
func (O *Orderbook_info) init() {
	O.wg.Add(1)
	O.orderbook_struct_temp = orderbook_infogather_struct{}
	O.infogather_original = make(chan []byte, 100)
	O.Infogather_processed = make(chan map[string]string, 100)
	O.ctx, O.cancel = context.WithCancel(context.Background())
	O.websocket_address = "wss://ws.okx.com:8443/ws/v5/public"
	file := "./log日志/" + time.Now().Format("20060102") + ".txt"
	logFile, err := os.OpenFile(file, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0766)
	if err != nil {
		panic(err)
	}
	O.logger = log.New(logFile, "[ok_quant:spot_goods_orderbook]", log.LstdFlags|log.Lshortfile|log.LUTC) // 将文件设置为loger作为输出

}

//建立wevsocket连接
func (O *Orderbook_info) start_websocket() {
	dialer := websocket.Dialer{}
	var err error
	O.connect, _, err = dialer.Dial(O.websocket_address, nil)
	if nil != err {
		O.logger.Println(err)
		return
	}
	O.logger.Println("websocket连接已建立")
}

//结束websocket连接
func (O *Orderbook_info) end_websocket() {
	O.connect.Close()
}

//向websocket订阅一个服务
func (O *Orderbook_info) subscribe_a_server() {
	i_f_subscribe := info_sender{Op: "subscribe", Args: []args_struct{{Channel: "books", InstID: O.Instrument_id}}}
	message_send, err := json.Marshal(i_f_subscribe)
	if err != nil {
		O.logger.Println("转为json文件失败：", err)
	}
	err = O.connect.WriteMessage(websocket.TextMessage, message_send)
	if err != nil {
		O.logger.Println("订阅失败", err)
	} else {
		O.logger.Println("订阅已申请,订阅json:", string(message_send))
	}
}

//向websocket取消一个订阅
func (O *Orderbook_info) unsubscribe_a_server() {
	i_f_unsubscribe := info_sender{Op: "unsubscribe", Args: []args_struct{{Channel: "books", InstID: O.Instrument_id}}}
	message_send, err := json.Marshal(i_f_unsubscribe)
	if err != nil {
		O.logger.Println("转为json文件失败：", err)
	}
	err = O.connect.WriteMessage(websocket.TextMessage, message_send)
	if err != nil {
		O.logger.Println("订阅取消失败", err)
	} else {
		O.logger.Println("订阅已申请取消,取消json:", string(message_send))
	}
}

//将收到的信息存放于特定channel中
func (O *Orderbook_info) save_in_channel() {
LOOP:
	for {
		messageType, messageData, err := O.connect.ReadMessage()
		if err != nil {
			O.logger.Println("websocket_get_err:", err)
		}
		switch messageType {
		case websocket.TextMessage: //文本数据
			O.infogather_original <- messageData
		case websocket.BinaryMessage: //二进制数据
			O.infogather_original <- messageData
		case websocket.CloseMessage: //关闭
		case websocket.PingMessage: //Ping
			O.connect.WriteMessage(websocket.PongMessage, messageData)
		case websocket.PongMessage: //Pong
		default:
			O.logger.Println("unkown_message_type:", messageType)
		}
		select {
		case <-O.ctx.Done():
			break LOOP
		default:
		}
	}
}

//将原始数据导入处理数据通道
func (O *Orderbook_info) convert_channel() {
	for {
		original_data := <-O.infogather_original
		json.Unmarshal(original_data, &O.orderbook_struct_temp)
		fmt.Println(O.orderbook_struct_temp)
	}
}

//用于结束通道
func (O *Orderbook_info) End_channel() {
	O.cancel()
	O.wg.Done()
	O.logger.Println("通道已关闭")
	O.unsubscribe_a_server()
	O.logger.Println("websocket握手已关闭")
}

//开始tiker信息的获取
func (O *Orderbook_info) Start_ticker_info() {
	O.init()
	O.start_websocket()
	O.subscribe_a_server()
	go O.save_in_channel()
	time.Sleep(time.Millisecond * 100)
	O.convert_channel()
}
