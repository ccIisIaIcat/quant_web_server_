...
type Feature_matrix struct {
	...
	start_matrix       [][]float64
	end_matrix         [][]float64
	divide_period      float64
	group_num          int
}
...
func (F *Feature_matrix) init() {
	...
	F.start_matrix = make([][]float64, 52)
	F.end_matrix = make([][]float64, 52)
...
func (F *Feature_matrix) make_data_long_matrix() {
	for i := 0; i < len(F.period_list); i++ {
...
func (F *Feature_matrix) list_divide(sample_list []float64, divide_period float64, group_num int) ([]float64, []float64) {
	sort.Float64s(sort.Float64Slice(sample_list))
	group_gap := (1 - divide_period) / float64(group_num)
	start_list := make([]float64, group_num+1)
	end_list := make([]float64, group_num+1)
	for i := 0; i <= group_num; i++ {
		start_list[i] = sample_list[int(math.Floor(float64(len(sample_list))*group_gap*float64(i)))]
		end_list[i] = sample_list[int(math.Floor(float64(len(sample_list))*(group_gap*float64(i)+divide_period)))-1]
...
func (F *Feature_matrix) make_start_and_end_matrix() {
	for i := 0; i < len(F.data_long_matrix); i++ {
		F.start_matrix[i], F.end_matrix[i] = F.list_divide(F.data_long_matrix[i], F.divide_period, F.group_num)
	}
}
...
func (F *Feature_matrix) make_divided_list(sample_list []float64, null_list *[]int) {
	for i := 0; i < len(sample_list); i++ {
		for order := 0; order < len(F.start_matrix[i]); order++ {
			if sample_list[i] <= F.end_matrix[i][order] && sample_list[i] >= F.start_matrix[i][order] {
				(*null_list)[i] = order + 1
				break
			}
		}
...
func (F *Feature_matrix) Start() {
	F.init()
	F.creat_table()
	F.make_data_long_matrix()
	F.make_start_and_end_matrix()
	F.save_classification_info()

}

func main() {
	f_m := Feature_matrix{File_name: "btcusdt2022_01_01_19h05m11s", Username: "root", Password: "", K_value_data: "classify_data", divide_period: 0.2, group_num: 10}
	f_m.Start()
}
